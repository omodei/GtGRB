...
class srcList:
	...
	def makeModel(self,GDfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/gll_iem_v06.fits",GDname='gll_iem_v06',ISOfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/iso_P8R2_SOURCE_V6_v06.txt",ISOname='iso_P8R2_SOURCE_V6_v06',normsOnly=False,extDir='',radLim=-1,maxRad=None,ExtraRad=10,sigFree=5,varFree=True,psForce=False,makeRegion=True,GIndexFree=False,ApplyEDisp=False,wd='',oldNames=False,Exposure=0):
		...
		if self.srcs.split('.')[-1]=='xml':
			...
			addSrcsFITS(self,GDfile,GDname,ISOfile,ISOname,oldNames,Exposure)
...
print "This is make3FGLxml version 01r01."
print "The default diffuse model files and names are for pass 8 and assume you have v10rxxpxx of the Fermi Science Tools or higher."
...
def addSrcsXML(sL,GD,GDn,ISO,ISOn,oldNames=False):
	...
	for src in catalog:
		...
		if dist<=sL.roi[2]+sL.ER:
			...
			try: excludeSource=(os.environ['EXCLUDESOURCE'] in sname)
			except:	excludeSource=False
			if excludeSource: 
				print '*** make2FGLxml: SKIPPING ',sname
				continue
...
def addSrcsFITS(sL,GD,GDn,ISO,ISOn,oldNames,Exposure):
	...
	Flux100_300=data.field('Flux100_300')
	Flux300_1000=data.field('Flux300_1000')
	Flux1000_3000=data.field('Flux1000_3000')
	Flux3000_10000=data.field('Flux3000_10000')
	Flux10000_100000=data.field('Flux10000_100000')
	Flux100_100000=Flux100_300+Flux300_1000+Flux1000_3000+Flux3000_10000+Flux10000_100000
	...
	for x in radii:
		if x==sL.roi[2]+sL.ER:
			...
			model.write('\n<!-- Sources between [%s,%s) degrees of ROI center -->\n' %(x-step,x))
		for n,f,i,r,d,p,c,t,b,TS,ei,vi,En,FL in zip(name,flux,index,ra,dec,pivot,cutoff,spectype,beta,Sigvals,expIndex,VarIdx,EName,Flux100_100000):
			...
			if Exposure>0 and Exposure*FL*6e4<1.0: continue
			try: excludeSource=(os.environ['EXCLUDESOURCE'] in n)
			except:	excludeSource=False
			if excludeSource: 
				print '*** make3FGLxml: SKIPPING ',n
				continue
