...
class srcList:
	...
	def makeModel(self,GDfile='$(GLAST_EXT)/extFiles/v0r9/galdiffuse/gll_iem_v02.fit',
		      GDname='GAL_v02',ISOfile=None,ISOname='Extragalactic Diffuse',
		      extDir='',radLim=-1,signif=4,psForce=False,Exposure=0): #the version number may differ, but default syntax from ModelEditor didn't work
		print '*** Make2FGLxml DEBUG: EXPOSURE=', Exposure
		...
		try:
			...
			use=1
		if use==1:
			addSrcs1(self,GDfile,GDname,ISOfile,ISOname,signif,Exposure)
		else:
			addSrcs2(self,GDfile,GDname,ISOfile,ISOname,signif,Exposure)
...
def addSrcs2(sL,GD,GDn,ISO,ISOn,signif,Exposure):
	...
	f100_300   = data.field('Flux100_300')
	f300_1000  = data.field('Flux300_1000')
	f1000_3000 = data.field('Flux1000_3000')
	flux_tot   = f100_300 + f300_1000 + f1000_3000
	...
	for x in radii:
		if x==sL.roi[2]+5.:
			...
			model.write('\n<!-- Sources between [%s,%s) degrees of ROI center -->\n' %(x-step,x))
		for n,f,i,r,d,p,c,t,b,S,EN,f_tot in zip(name,flux,index,ra,dec,pivot,cutoff,spectype,beta,sigma,ExtName,flux_tot):
			dist=angsep(sL.roi[0],sL.roi[1],r,d) #check that source is within ROI radius + 5 degress of ROI center
			if Exposure>0 and Exposure*f_tot*6e4<1.0:continue
			#print os.environ['EXCLUDESOURCE'],n,os.environ['EXCLUDESOURCE'] in n
			try: excludeSource=(os.environ['EXCLUDESOURCE'] in n)
			except:	excludeSource=False
			if excludeSource: 
				print '*** make2FGLxml: SKIPPING ',n
				continue
			
			if r==sL.roi[0] and d==sL.roi[1]:
				dist=0.0
			if (dist<x and dist>=x-step) or (x==sL.roi[2]+5. and dist==x):
				print '*** make2FGLxml:', n,dist,Exposure*f_tot*6e4							
				sn=''
				for N in n.split(' '):
					sn+=N
...
def addSrcs1(sL,GD,GDn,ISO,ISOn,signif,Exposure):
	model=open(sL.out,'w') #open file in write mode, overwrites other files of same name
	file=pyfits.open(sL.srcs) #open source list file and access necessary fields, requires LAT source catalog definitions and names
	data=file[1].data
	try:
	...
	for x in radii:
		if x==sL.roi[2]+5.:
			...
			model.write('\n<!-- Sources between [%s,%s) degrees of ROI center -->\n' %(x-step,x))
		for n,f,i,r,d,p,s,f_tot in zip(name,flux,index,ra,dec,pivot,sigma,flux_tot):
			dist=angsep(sL.roi[0],sL.roi[1],r,d) #check that source is within ROI radius + 5 degress of ROI center
			if Exposure>0 and Exposure*f_tot*6e4<1.0:continue
